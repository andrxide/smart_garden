...
public class Garden
{
    ...
    public Garden(string id, DateTime createdAt, string name, string description, Boolean online)
    {
        Id = id;
        CreatedAt = createdAt;
        Name = name;
    ...
    public static List<Garden> GetAll(int userId)
    {
        ...
        foreach (DataRow row in table.Rows)
        {
            var online = false;
            if ((int)row["g_online"] == 1) online = true;
            list.Add(new Garden(row["g_ID"].ToString(), DateTime.Parse(row["g_createdAt"].ToString()), row["g_name"].ToString(), row["g_description"].ToString(),online));
        }
        return list;
    }
    #endregion
